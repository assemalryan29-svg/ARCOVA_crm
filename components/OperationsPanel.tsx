...
export default function OperationsPanel({ currentUser, userRole, leads = [], units = [] }) {
  ...
  const Input = ({ name, placeholder, type = 'text', required = false }) => <input name={name} type={type} placeholder={placeholder} required={required} style={{ padding: '0.45rem', background: '#fffaf0', color: '#3f321f', border: '1px solid #d9c5a4', borderRadius: '5px', fontSize: '0.78rem' }} />;
  const selectStyle = { padding: '0.45rem', background: '#fffaf0', color: '#3f321f', border: '1px solid #d9c5a4', borderRadius: '5px', fontSize: '0.78rem' };

  return (
    <div style={{ display: 'grid', gap: '0.9rem' }}>
      <div style={{ display: 'flex', gap: '0.45rem', flexWrap: 'wrap' }}>
        {tabs.filter((t) => t.key !== 'finance' || canFinanceView).map((t) => <button key={t.key} type='button' onClick={() => setActive(t.key)} style={{ padding: '0.5rem 0.8rem', background: active === t.key ? '#b08a4a' : '#3f321f', color: active === t.key ? '#fffaf0' : '#b08a4a', border: '1px solid #b08a4a', borderRadius: '5px', cursor: 'pointer' }}>{t.label}</button>)}
      </div>
      ...
      {active === 'calls' && <div style={{ display:'grid',gap:'0.7rem' }}>
        ...
        <div style={{ display:'grid',gap:'0.5rem' }}>{calls.map((c)=><div key={c.id} style={{ background:'#3f321f',border:'1px solid #d9c5a4',borderRadius:'7px',padding:'0.7rem',fontSize:'0.78rem' }}>{c.leads?.name||'—'} · {c.outcome||'بدون نتيجة'} · {c.notes||''}</div>)}</div>
      </div>}
      ...
      {active === 'appointments' && <div style={{ display:'grid',gap:'0.7rem' }}>
        ...
        <div style={{ display:'grid',gap:'0.5rem' }}>{appointments.map((a)=><div key={a.id} style={{ background:'#3f321f',border:'1px solid #d9c5a4',borderRadius:'7px',padding:'0.7rem',fontSize:'0.78rem' }}>{a.leads?.name||'—'} · {new Date(a.scheduled_at).toLocaleString('ar-EG')} · {a.status}</div>)}</div>
      </div>}
