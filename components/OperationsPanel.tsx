...
export default function OperationsPanel({ currentUser, userRole, leads = [], units = [] }) {
  ...
  const Input = ({ name, placeholder, type = 'text', required = false }) => <input name={name} type={type} placeholder={placeholder} required={required} style={{ padding: '0.45rem', background: '#3f321fdf8', color: '#3f321f', border: '1px solid #d9c5a4', borderRadius: '5px', fontSize: '0.78rem' }} />;
  const selectStyle = { padding: '0.45rem', background: '#3f321fdf8', color: '#3f321f', border: '1px solid #d9c5a4', borderRadius: '5px', fontSize: '0.78rem' };

  return (
    <div style={{ display: 'grid', gap: '0.9rem' }}>
      <div style={{ display: 'flex', gap: '0.45rem', flexWrap: 'wrap' }}>
        {tabs.filter((t) => t.key !== 'finance' || canFinanceView).map((t) => <button key={t.key} type='button' onClick={() => setActive(t.key)} style={{ padding: '0.5rem 0.8rem', background: active === t.key ? '#b08a4a' : '#3f321faf0', color: active === t.key ? '#3f321fdf8' : '#b08a4a', border: '1px solid #b08a4a', borderRadius: '5px', cursor: 'pointer' }}>{t.label}</button>)}
      </div>

      {active === 'deals' && <div style={{ display: 'grid', gap: '0.7rem' }}>
        {canOperate && <form onSubmit={(e) => { const f=new FormData(e.currentTarget); return submit(e,'deals',{lead_id:f.get('lead_id'),unit_id:f.get('unit_id')||null,sales_person:currentUser.id,deal_value:Number(f.get('deal_value')||0),down_payment:Number(f.get('down_payment')||0),installment_months:Number(f.get('installment_months')||0),payment_frequency:f.get('payment_frequency'),status:f.get('status'),notes:f.get('notes')||null},'فشل إنشاء الصفقة'); }} style={{ background:'#3f321faf0',padding:'0.8rem',borderRadius:'8px',border:'1px solid #d9c5a4',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:'0.45rem' }}>
        ...
        <div style={{ display:'grid',gap:'0.5rem' }}>{deals.map((d)=><div key={d.id} style={{ background:'#3f321faf0',border:'1px solid #d9c5a4',borderRadius:'7px',padding:'0.7rem',fontSize:'0.78rem' }}><strong>{d.leads?.name || '—'}</strong> · {Number(d.deal_value || 0).toLocaleString()} ج · {d.status}</div>)}{!deals.length&&<div style={{color:'#9a7b4b'}}>لا توجد صفقات.</div>}</div>
      </div>}
      ...
      {active === 'reservations' && <div style={{ display:'grid',gap:'0.7rem' }}>
        ...
        <div style={{ display:'grid',gap:'0.5rem' }}>{reservations.map((r)=><div key={r.id} style={{ background:'#3f321faf0',border:'1px solid #d9c5a4',borderRadius:'7px',padding:'0.7rem',fontSize:'0.78rem' }}>{r.leads?.name||'—'} · {r.units?.unit_number||r.units?.title||'—'} · {Number(r.reservation_amount||0).toLocaleString()} ج · {r.status}</div>)}</div>
      </div>}
      ...
      {active === 'calls' && <div style={{ display:'grid',gap:'0.7rem' }}>
        {canOperate && <form onSubmit={(e)=>{const f=new FormData(e.currentTarget);return submit(e,'calls',{lead_id:f.get('lead_id'),assigned_to:currentUser.id,call_at:f.get('call_at')?new Date(f.get('call_at')).toISOString():new Date().toISOString(),duration_seconds:Number(f.get('duration_seconds')||0),outcome:f.get('outcome')||null,notes:f.get('notes')||null},'فشل تسجيل المكالمة');}} style={{ background:'#3f321faf0',padding:'0.8rem',borderRadius:'8px',border:'1px solid #d9c5a4',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:'0.45rem' }}>
          ...
          <Input name='notes' placeholder='ملاحظات' />
          <button disabled={busy} type='submit' style={{ background:'#b08a4a',border:0,borderRadius:'5px',fontWeight:700 }}>تسجيل المكالمة</button>
        </form>}
        <div style={{ display:'grid',gap:'0.5rem' }}>{calls.map((c)=><div key={c.id} style={{ background:'#3f321faf0',border:'1px solid #d9c5a4',borderRadius:'7px',padding:'0.7rem',fontSize:'0.78rem' }}>{c.leads?.name||'—'} · {c.outcome||'بدون نتيجة'} · {c.notes||''}</div>)}</div>
      </div>}
      ...
      {active === 'appointments' && <div style={{ display:'grid',gap:'0.7rem' }}>
        {canOperate && <form onSubmit={(e)=>{const f=new FormData(e.currentTarget);return submit(e,'appointments',{lead_id:f.get('lead_id'),assigned_to:currentUser.id,scheduled_at:new Date(f.get('scheduled_at')).toISOString(),type:f.get('type')||'Meeting',status:'Planned',notes:f.get('notes')||null},'فشل إنشاء الموعد');}} style={{ background:'#3f321faf0',padding:'0.8rem',borderRadius:'8px',border:'1px solid #d9c5a4',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:'0.45rem' }}>
          ...
          <Input name='notes' placeholder='ملاحظات' />
          <button disabled={busy} type='submit' style={{ background:'#b08a4a',border:0,borderRadius:'5px',fontWeight:700 }}>حفظ الموعد</button>
        </form>}
        <div style={{ display:'grid',gap:'0.5rem' }}>{appointments.map((a)=><div key={a.id} style={{ background:'#3f321faf0',border:'1px solid #d9c5a4',borderRadius:'7px',padding:'0.7rem',fontSize:'0.78rem' }}>{a.leads?.name||'—'} · {new Date(a.scheduled_at).toLocaleString('ar-EG')} · {a.status}</div>)}</div>
      </div>}
      ...
      {active === 'finance' && canFinanceView && <div style={{ display:'grid',gap:'0.7rem' }}>
        ...
        <div style={{ display:'grid',gap:'0.5rem' }}>{payments.map((p)=><div key={p.id} style={{ background:'#3f321faf0',border:'1px solid #d9c5a4',borderRadius:'7px',padding:'0.7rem',fontSize:'0.78rem' }}>قسط {p.installment_no} · {Number(p.amount||0).toLocaleString()} ج · {p.status} · {p.due_date}</div>)}</div>
      </div>}
    </div>
  );
}