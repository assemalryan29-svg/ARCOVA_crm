...
export default function Sidebar() {
  ...
  return (
    <aside
      dir="rtl"
      className="flex min-h-screen w-64 shrink-0 flex-col justify-between border-l border-slate-800 bg-slate-950 p-4 text-white shadow-xl"
    >
      <div>
        <div className="mb-8 flex items-center gap-3 border-b border-slate-800 px-2 pb-5 pt-2">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl border border-amber-400/70 bg-gradient-to-br from-amber-300 to-amber-600 text-xl font-bold text-slate-950 shadow-lg shadow-amber-500/10">
            A
          </div>
          <div>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <ChevronLeft
                  className={`h-4 w-4 transition-opacity ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-60'}`}
                />
              </Link>
            );
          })}
