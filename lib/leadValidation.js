 ...
 * Client-side validation is a UX layer; the database/API remains authoritative.
 */

import { isValidLeadStatus } from './leadStatuses';
...
export function validateLeadInput(lead = {}) {
  ...
  if (lead.status && !isValidLeadStatus(lead.status)) errors.status = 'حالة العميل غير صحيحة';
