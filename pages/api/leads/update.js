...
const EDIT_ROLES = new Set(['admin', 'ceo', 'manager', 'team_leader', 'sales', 'marketing']);
...
export default async function handler(req, res) {
  ...
  if (!url || !serviceRoleKey) return res.status(500).json({ error: 'Server is not configured.' });

  const token = String(req.headers.authorization || '').replace(/^Bearer\s+/i, '');
  if (!token) return res.status(401).json({ error: 'Missing authorization token.' });

  const adminClient = client(serviceRoleKey);
  const publicClient = client(process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY);
  ...
  if (!roleRow?.active || !EDIT_ROLES.has(role)) return res.status(403).json({ error: 'Insufficient permissions.' });
