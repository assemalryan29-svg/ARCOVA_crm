...
const checks = [
  ...
  ['gateway', 'deals are protected from direct CRUD gateway', (source) => !/\bdeals:\s*\{/.test(source)],
  ['gateway', 'reservations are protected from direct CRUD gateway', (source) => !/\breservations:\s*\{/.test(source)],
  ['gateway', 'payments are protected from direct CRUD gateway', (source) => !/\bdeal_payments:\s*\{/.test(source)],
