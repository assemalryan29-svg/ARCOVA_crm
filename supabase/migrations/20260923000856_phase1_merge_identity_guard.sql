...
declare
  ...
  v_primary_phone_normalized text;
  v_primary_email_normalized text;
begin
  ...
  v_primary_phone_normalized := nullif(v_primary.phone_normalized, '');
  v_primary_email_normalized := nullif(v_primary.email_normalized, '');

  if not (
    (v_primary_phone_normalized is not null
      and (select count(*) from public.leads d where d.id = any(p_duplicate_lead_ids)
           and d.phone_normalized = v_primary_phone_normalized) = v_duplicate_count)
    or
    (v_primary_email_normalized is not null
      and (select count(*) from public.leads d where d.id = any(p_duplicate_lead_ids)
           and d.email_normalized = v_primary_email_normalized) = v_duplicate_count)
  ) then
    raise exception using errcode = '22023', message = 'Selected leads do not form the same duplicate identity group.';
  end if;
